...
namespace GameWorld
{
    ...
    /// <summary>
    /// 网格数据缓存
    /// 如果烘焙同一 sharedMesh 的许多实例，则缓存这些结果，而不是从网格中多次获取它们
    /// </summary>
    ...
    {
         MeshCombineHandler mc;
        protected Dictionary<int, MeshChannels> meshIDToMeshChannels = new Dictionary<int, MeshChannels>();

        public MeshChannelsCache(MeshCombineHandler mcs)
        {
            mc = mcs;
        }
        ...
        internal Vector2[] GetUv3(Mesh m)
        {
            ...
            if (mc.uv3 == null)
            {
                mc.uv3 = MeshBakerUtility.GetMeshUV3orUV4(m, true);
            }
            return mc.uv3;
        }
        ...
        Vector2[] _getMeshUV2s(Mesh m)
        {
            ...
            if (uv.Length == 0)
            {
                ...
                if (mc.settingsHolder.GetMeshBakerSettings().lightmapOption == LightmapOptions.copy_UV2_unchanged_to_separate_rects)
