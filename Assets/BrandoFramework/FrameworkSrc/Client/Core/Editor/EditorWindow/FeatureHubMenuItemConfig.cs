...
{ 
    ...
    /// </summary>
[Serializable]
    public class FeatureHubMenuItemConfig
    {
        [ShowInInspector]
        [ListDrawerSettings(Expanded = true,DraggableItems = true,HideRemoveButton = true)]
        [TableList(IsReadOnly = true,CellPadding = 3)]
        [LabelText("菜单选项配置列表")]
        private readonly List<MenuItemPriorityItem> PriorityItems = new List<MenuItemPriorityItem>();

        private List<IMenuEditorWindowItemBuilder> builders;

        public void SetMenuPriorityUp(MenuItemPriorityItem item)
        {
    ...
    [Serializable]
    public class MenuItemPriorityItem
    {
        [LabelWidth(120)]
        ...
        [HideInInspector]
        public Action<MenuItemPriorityItem> ItemUpAction;
        ...
        public MenuItemPriorityItem(string buidlerDesc, string builderId)
        {
