...
namespace Common.PrefsData
{
        ...
        where T : class, new()
    {
                ...
                if (YuUnityUtility.IsEditorMode)
                {
                    if (isEditorSetting)
                    {
                        ...
                        if (!File.Exists(editorPath))
                        {
                            ...
                            instance = new T();
                            var jsContent = UnityEngine.JsonUtility.ToJson(instance);
                            IOUtility.WriteAllText(editorPath, jsContent);
                            return instance;
                        }

                        var jsonContent = File.ReadAllText(editorPath);
                        instance = UnityEngine.JsonUtility.FromJson<T>(jsonContent);
                    }
                    else
                    {
                        ...
                        instance = UnityEngine.JsonUtility.FromJson<T>(textAsset.text) ?? new T();
                    }
                }
                else
                {
                    var resId = "Setting/" + typeof(T).Name;
                    var textAsset = Resources.Load<TextAsset>(resId);
                    instance = UnityEngine.JsonUtility.FromJson<T>(textAsset.text);
                }
        ...
        private void Reload()
        {
            ...
            var t = UnityEngine.JsonUtility.FromJson<T>(jsonContent);
            instance = t;
        }
