...
{
    public class MeshCombineSettingEditor
    {
        private static GUIContent
            gc_renderTypeGUIContent = new GUIContent("渲染器类型", "The type of renderer to add to the combined mesh."),
            gc_lightmappingOptionGUIContent = new GUIContent("光照贴图 UV 设置", "preserve current lightmapping: Use this if all source objects are lightmapped and you want to preserve it. " +
            ...
            gc_clearBuffersAfterBakeGUIContent = new GUIContent("合并后清空缓存", "Frees memory used by the MeshCombiner. Set to false if you want to update the combined mesh at runtime."),
            gc_doNormGUIContent = new GUIContent("计算法线"),
            gc_doTanGUIContent = new GUIContent("计算切线"),
            gc_doColGUIContent = new GUIContent("计算 Colors"),
            gc_doBlendShapeGUIContent = new GUIContent("计算 Blend Shapes"),
            gc_doUVGUIContent = new GUIContent("计算 UV"),
            gc_doUV3GUIContent = new GUIContent("计算 UV3"),
            gc_doUV4GUIContent = new GUIContent("计算 UV4"),
            ...
            gc_CenterMeshToBoundsCenter = new GUIContent("Center Mesh To Render Bounds", 
                "Centers the verticies of the mesh about the render bounds center and translates the game object. " +
                "This makes the combined meshes easier to work with. " +
                "There is a performance and memory allocation cost to this " +
                "so if you are frequently baking meshes at runtime disable it."),
            gc_OptimizeAfterBake = new GUIContent("合并后优化模型", "This does the same thing that 'Optimize' does on the ModelImporter.");
