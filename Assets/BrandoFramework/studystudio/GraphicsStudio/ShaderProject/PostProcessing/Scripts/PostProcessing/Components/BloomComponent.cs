    ...
    public sealed class BloomComponent : PostProcessingComponentRenderTexture<BloomModel>
    {
        ...
        private float RealIntensity
        {
            get {
                ...
                if (SceneShaderController.Instance != null)
                {
                    lightStrength = SceneShaderController.Instance.m_LightStrength;
                }

                return model.settings.bloom.intensity * lightStrength;
            }
        }
        ...
        public void Prepare(RenderTexture source, Material uberMaterial, Texture autoExposure)
        {
            ...
            // Prefilter pass
            var prefiltered = context.renderTextureFactory.Get(tw, th, 0, rtFormat, "BloomComponent_Prefilter");
            ...
            for (int level = 0; level < iterations; level++)
            {
                m_BlurBuffer1[level] = context.renderTextureFactory.Get(
                        last.width / 2, last.height / 2, 0, rtFormat, "BloomComponent_BlurBuffer1"
                        );
            ...
            for (int level = iterations - 2; level >= 0; level--)
            {
                ...
                m_BlurBuffer2[level] = context.renderTextureFactory.Get(
                        baseTex.width, baseTex.height, 0, rtFormat, "BloomComponent_BlurBuffer2"
                        );
            ...
            uberMaterial.SetVector(Uniforms._Bloom_Settings, new Vector2(sampleScale, RealIntensity));
