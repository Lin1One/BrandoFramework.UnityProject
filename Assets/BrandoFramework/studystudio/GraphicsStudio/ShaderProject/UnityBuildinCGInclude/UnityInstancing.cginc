...
////////////////////////////////////////////////////////
// basic instancing setups 基本实例设置
// - 在顶点着色器输入/输出结构中声明实例ID字段。
...
// - UNITY_GET_INSTANCE_ID              (Internal) Get the instance ID from input struct.
    ...
    // Don't make UnityDrawCallInfo an actual CB on GL
        ...
        int unity_BaseInstanceID;   //实例ID
        int unity_InstanceCount;    //实例数量
    ...
    #else
        #define DEFAULT_UNITY_VERTEX_INPUT_INSTANCE_ID 
            uint instanceID : SV_InstanceID;    //声明实例ID
        #define UNITY_GET_INSTANCE_ID(input)    
            input.instanceID
    #endif
#else
...
// - UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX
    ...
    #define DEFAULT_UNITY_VERTEX_OUTPUT_STEREO 
        float stereoTargetEyeIndex : BLENDWEIGHT0;
    ...
    DEFAULT_UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(input, output)
#endif
...
// instanced property arrays
    ...
    // 以下矩阵求值取决于静态变量 unity_InstanceID 和 unity_StereoEyeIndex。 
    // 它们需要在UnitySetupInstanceID之后初始化。
