...
#define UNITY_SHOULD_SAMPLE_SH 
    (defined(LIGHTPROBE_SH) && 
    !defined(UNITY_PASS_FORWARDADD) && 
    !defined(UNITY_PASS_PREPASSBASE) && 
...
#define TANGENT_SPACE_ROTATION \
    ...
    // rotation为3x3的矩阵，由切线空间的基组成。可以使用它把物体空间转换到切线空间中。
...
#define UNITY_OPAQUE_ALPHA(outputAlpha) 
    outputAlpha = 1.0
...
#define UNITY_FOG_COORDS_PACKED(idx, vectype) 
    vectype fogCoord : TEXCOORD##idx;

//【1】若已经定义了FOG_LINEAR、FOG_EXP、FOG_EXP2宏三者之中至少之一，便可以进行到此#if实现部分
#if defined(FOG_LINEAR) || defined(FOG_EXP) || defined(FOG_EXP2)
    //【1-1】定义UNITY_FOG_COORDS(idx)宏
    #define UNITY_FOG_COORDS(idx) 
        UNITY_FOG_COORDS_PACKED(idx, float1)
    ...
    #else
        // Shader Model 3.0和PC/游戏主机平台：计算每顶点的雾距离，以及每像素雾效因子 
        // SM3.0 and PC/console: calculate fog distance per-vertex, and fog factor per-pixel
        #define UNITY_TRANSFER_FOG(o,outpos) 
            o.fogCoord.x = (outpos).z
...
#if defined(FOG_LINEAR) || defined(FOG_EXP) || defined(FOG_EXP2)
    //【1-1】若满足着色目标模型的版本小于Shader Model 3.0，
    // 或者定义了SHADER_API_MOBILE宏，便可以进行到此
    #if (SHADER_TARGET < 30) || defined(SHADER_API_MOBILE)
        // mobile or SM2.0: fog factor was already calculated per-vertex, so just lerp the color
        //移动平台和Shader Model 2.0：已经计算了每顶点的雾效因子，所以插一下值就可以了
        #define UNITY_APPLY_FOG_COLOR(coord,col,fogCol) 
            UNITY_FOG_LERP_COLOR(col,fogCol,(coord).x)
    #else
        // SM3.0 and PC/console: calculate fog factor and lerp fog color
        //【1-2】 Shader Model 3.0和PC/游戏主机平台：计算雾效因子以及进行雾颜色的插值
        #define UNITY_APPLY_FOG_COLOR(coord,col,fogCol) 
            UNITY_CALC_FOG_FACTOR((coord).x); 
            UNITY_FOG_LERP_COLOR(col,fogCol,unityFogFactor)
    #endif
    #define UNITY_EXTRACT_FOG(name) 
        float _unity_fogCoord = name.fogCoord
    #define UNITY_EXTRACT_FOG_FROM_TSPACE(name) 
        float _unity_fogCoord = name.tSpace2.y
    #define UNITY_EXTRACT_FOG_FROM_WORLD_POS(name) 
        float _unity_fogCoord = name.worldPos.w
    #define UNITY_EXTRACT_FOG_FROM_EYE_VEC(name) 
        float _unity_fogCoord = name.eyeVec.w
#else
//【2】否则，直接定义UNITY_APPLY_FOG_COLOR宏对应空
    #define UNITY_APPLY_FOG_COLOR(coord,col,fogCol)
    #define UNITY_EXTRACT_FOG(name)
    #define UNITY_EXTRACT_FOG_FROM_TSPACE(name)
    #define UNITY_EXTRACT_FOG_FROM_WORLD_POS(name)
